...
export function CalculatorSection({ lang }: { lang: Lang }) {
  ...
  return (
    <section id="calculator" className="bg-[#F4F6FB] py-24">
      <Container>
        <div>
          <div className="mx-auto max-w-2xl text-center">
            ...
            <h2 className="mt-4 text-balance text-3xl font-extrabold tracking-tight text-slate-900 md:text-5xl">
              {t(lang, 'calc_title')}
            </h2>
            <p className="mx-auto mt-3 max-w-[32rem] text-pretty text-[15px] leading-7 text-slate-600">
              {t(lang, 'calc_desc')}
            </p>
          </div>

          <div className="mx-auto mt-10 grid max-w-6xl gap-6 md:grid-cols-2 md:items-stretch">
            <div className="flex h-full flex-col rounded-[20px] border border-slate-200/80 bg-white p-8 shadow-[0_2px_20px_rgba(0,0,0,0.04)]">
              <div>
                <div>
              ...
              <input
                ...
                className="calc-range mt-6 w-full shrink-0 cursor-pointer appearance-none"
                style={{
                  background: `linear-gradient(to right, #1d4ed8 ${sliderPct}%, #E2E7F0 ${sliderPct}%)`,
                }}
              />

              <div className="relative mt-[22px] min-h-[180px] flex-1 overflow-hidden rounded-xl bg-slate-900">
                {shots.map((src, i) => (
                  <img
                    key={src}
                    src={src}
            ...
            <div className="rounded-[20px] border border-slate-200/80 bg-white p-8 shadow-[0_2px_20px_rgba(0,0,0,0.04)]">
